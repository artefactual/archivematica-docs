...
at the Normalization micro-service. For more information please see
...
A content management tool called `AtoM`_  is
...
Access integration workflows also exist for `ArchivesSpace`_,
`Archivists' Toolkit`_ , and `ContentDM`_."
...
:ref:`Administrator manual - AtoM DIP upload <admin-dashboard-atom>` for more
information.
...
3. Click the blue "Upload" button. Digital objects are uploaded as items within
   the description to which the DIP is being uploaded. If you want to create a
   child level of description under the target description, you must add the
   title of that level of description using the DC metadata template prior to
   normalization.
...
7. The digital object is displayed in AtoM. Clicking on the image will open
   the uploaded object.
...
workflow if you have digital objects which you wish to make discoverable, but
not displayed online for copyright or privacy reasons.
...
.. note::
   ...
   * Descriptive metadata: if descriptive metadata is included by csv or
     ...
     in this workflow.
   * SIPs with levels of description assigned using the
     :ref:`arranging for AtoM <hierarchical-dip>` workflow- the levels of
     description will be ignored in this workflow.
