...
2. The Storage Service has its own set of users. Navigate to
   *Administration > Users* and add at least one administrative user. After
   ...
   the Archivematica pipeline to the Storage Service API. Click edit on the new
   user. The API key will be found at the bottom of the page. Copy this to
   your clipboard as this will be used later on in the post-installation
   configuration.
