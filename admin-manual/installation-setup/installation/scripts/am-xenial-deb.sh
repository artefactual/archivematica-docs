...
sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.10.x/ubuntu xenial main" >> /etc/apt/sources.list'
sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.10.x/ubuntu-externals xenial main" >> /etc/apt/sources.list'
