...
sudo wget -O - https://packages.archivematica.org/1.11.x/key.asc | sudo apt-key add -

sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.11.x/ubuntu xenial main" >> /etc/apt/sources.list'
sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.11.x/ubuntu-externals xenial main" >> /etc/apt/sources.list'
...
sudo apt-get install -y wget htop ntp unzip apt-transport-https openjdk-8-jre-headless
