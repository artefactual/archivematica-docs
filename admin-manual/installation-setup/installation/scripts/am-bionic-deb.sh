...
sudo wget -O - https://packages.archivematica.org/1.14.x/key.asc | sudo apt-key add -

sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.14.x/ubuntu bionic main" >> /etc/apt/sources.list'
sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.14.x/ubuntu-externals bionic main" >> /etc/apt/sources.list'
