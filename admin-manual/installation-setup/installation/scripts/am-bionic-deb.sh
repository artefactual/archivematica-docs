...
sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.11.x/ubuntu bionic main" >> /etc/apt/sources.list'
sudo sh -c 'echo "deb [arch=amd64] http://packages.archivematica.org/1.11.x/ubuntu-externals bionic main" >> /etc/apt/sources.list'
